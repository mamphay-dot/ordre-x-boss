...
function openPlus(){
  ...
  sheet.innerHTML=`
    ...
    <button class="plus-item gold" id="pl-strategie" style="background:linear-gradient(135deg,#241f10 0%,#1a1608 100%);border:2px solid var(--gold);color:var(--gold);font-weight:700">🎯 Ma stratégie business (coach IA)</button>
...
function renderAuthSheet(sheet){
  ...
  sheet.innerHTML = `
    ...
    <div class="auth-tabs">
      <button class="auth-tab ${t==='phone'?'on':''}" data-t="phone">📱 Téléphone</button>
      <button class="auth-tab ${t==='email'?'on':''}" data-t="email">✉️ Email</button>
    </div>

    ${t==='phone' ? `
      <div class="auth-method">
        <button class="auth-m ${method==='password'?'on':''}" data-m="password">🔐 Mot de passe</button>
        <button class="auth-m ${method==='sms'?'on':''}" data-m="sms">💬 Code SMS</button>
      </div>
    ...
    ` : `
      ...
      <div class="auth-method">
        <button class="auth-m ${method==='magic'?'on':''}" data-m="magic">✨ Lien magique</button>
        <button class="auth-m ${method==='password'?'on':''}" data-m="password">🔐 Mot de passe</button>
      </div>
...
/* ============================================================
   ...
   ============================================================ */
const BMC_BLOCKS = [
  { key:"segments", num:1, emoji:"👥", title:"Qui achète chez toi ?", sub:"Décris tes clients : qui sont-ils, où habitent-ils, âge, situation…", ph:"Ex : jeunes du quartier Yopougon, mamans qui vont au marché, ouvriers du chantier à côté, écoliers du collège Saint-Paul…", voice:"Dis-moi qui sont tes clients. Qui achète chez toi ?", examples:["Jeunes 18-30 ans du quartier","Mamans qui font les commissions","Étudiants","Ouvriers, chauffeurs"] },
  { key:"value", num:2, emoji:"⭐", title:"Pourquoi ils achètent chez toi ?", sub:"Qu'est-ce qui te rend différent du voisin ? Prix, qualité, gentillesse, rapidité ?", ph:"Ex : mon poulet est le plus tendre du quartier, je livre en moins de 20 min, mes prix sont les plus bas, je fais crédit aux fidèles…", voice:"Pourquoi tes clients achètent chez toi et pas chez le voisin ?", examples:["Prix les plus bas","Meilleure qualité","Livraison rapide","Bonne accueil, sourire"] },
  { key:"channels", num:3, emoji:"📢", title:"Comment tu les trouves ?", sub:"Où tes clients te découvrent ? Où tu les rencontres ?", ph:"Ex : bouche à oreille, WhatsApp Status, Facebook, Instagram, ma boutique au bord de la route, marché du samedi, panneau devant chez moi…", voice:"Comment tes clients te trouvent ? Où tu les rencontres ?", examples:["Bouche à oreille","Ma boutique visible","WhatsApp Status","Marché hebdomadaire"] },
  { key:"relations", num:4, emoji:"🤝", title:"Comment tu les fidélises ?", sub:"Comment tu fais pour qu'ils reviennent ? Cartes fidélité, cadeaux, SMS ?", ph:"Ex : je connais leur prénom, je fais crédit aux réguliers, je donne un cadeau après 10 achats, je les appelle quand j'ai du nouveau…", voice:"Comment tu fais pour que tes clients reviennent ?", examples:["Je connais leurs noms","Crédit pour les réguliers","Petit cadeau après 10 achats","SMS quand du nouveau arrive"] },
  { key:"revenue", num:5, emoji:"💰", title:"Comment tu gagnes de l'argent ?", sub:"Vente directe, livraison, abonnement, commission ?", ph:"Ex : je vends chaque plat 2 500 F, livraison payante 500 F, table réservée le week-end 5 000 F, abonnement à mes plats 30 000 F/mois…", voice:"Comment tu gagnes de l'argent ?", examples:["Vente au comptoir","Livraison à domicile","Commande à l'avance","Abonnement mensuel"] },
  { key:"resources", num:6, emoji:"🛠️", title:"Qu'est-ce qui te sert le plus ?", sub:"Ta machine, ton local, ta recette, ton fournisseur, ton talent ?", ph:"Ex : ma machine à couture Singer, mon four à charbon, ma recette de sauce graine, mon fournisseur du grand marché, mon savoir-faire coiffure…", voice:"Qu'est-ce qui te sert le plus dans ton business ?", examples:["Ma machine","Mon local bien placé","Ma recette","Mon savoir-faire"] },
  { key:"activities", num:7, emoji:"🔄", title:"Qu'est-ce que tu fais tous les jours ?", sub:"Les 3-4 activités qui prennent le plus de ton temps.", ph:"Ex : je fais le marché à 6h, je cuisine jusqu'à midi, je sers jusqu'à 22h, je fais les comptes le soir. Ou : je couds la journée, je livre à moto le soir…", voice:"Qu'est-ce que tu fais tous les jours pour ton business ?", examples:["Faire le marché","Cuisiner / préparer","Servir / vendre","Livrer, encaisser"] },
  { key:"partners", num:8, emoji:"🤲", title:"Qui te fournit ? Qui t'aide ?", sub:"Fournisseurs, associés, transporteurs, prêteur, famille.", ph:"Ex : ma sœur qui vend au marché, le boucher du coin qui me fournit la viande, le tenancier de la boutique d'à côté, un livreur avec sa moto…", voice:"Qui te fournit ou t'aide dans ton business ?", examples:["Fournisseurs du marché","Ma famille","Un livreur","La boutique à côté"] },
  { key:"costs", num:9, emoji:"💸", title:"Qu'est-ce que tu payes chaque mois ?", sub:"Loyer, salaires, matières premières, transport, électricité.", ph:"Ex : loyer 80 000 F, salaires 150 000 F, gaz et charbon 40 000 F, électricité 15 000 F, transport 20 000 F, marchandise 300 000 F…", voice:"Qu'est-ce que tu payes chaque mois pour ton business ?", examples:["Loyer","Salaires","Matières premières","Transport, électricité"] }
];
...
function openBmcIntro(){
  const sheet = $("#sheet");
  sheet.innerHTML = `
    <div class="sheet-head"><h3>🎯 Ma stratégie business</h3><button class="x" id="sheet-close">×</button></div>
    ...
    <button class="sheet-add" id="bmc-start" style="margin-top:14px">🚀 Commencer les 9 questions</button>
...
function openBmcQuestion(idx){
  ...
  sheet.innerHTML = `
    <div class="sheet-head">
      <h3><span style="color:var(--gold);font-family:'Archivo';font-weight:900">${q.num}/9</span> · ${q.emoji} ${escapeHtml(q.title)}</h3>
    ...
    ${showMic ? `<button class="easy-mic" id="bmc-mic" style="margin-top:8px">🎙️ Dicter ma réponse</button>` : ""}
    <button class="easy-mic" id="bmc-listen" style="margin-top:6px;background:none;border:1px solid var(--line);color:var(--cream)">🔊 Écouter la question</button>
  ...
  if(micBtn) micBtn.onclick = ()=>{
    ...
    EasyMode.listen(txt=>{
      ...
      saveField();
    }, ()=>{ micBtn.classList.remove("rec"); micBtn.textContent = "🎙️ Dicter ma réponse"; });
  };
...
function openBmcSummary(){
  ...
  sheet.innerHTML = `
    ...
    <button class="sheet-add" id="bmc-gen" style="margin-top:12px;font-size:16px;padding:16px">🤖 Générer ma stratégie avec l'IA</button>
    ...
    <button class="plus-item" id="bmc-review" style="margin-top:10px">📝 Revoir mes réponses avant</button>
  `;
  $("#sheet-close").onclick = closeSheet;
...
function openBmcView(){
  ...
  const cells = BMC_BLOCKS.map(q=>{
    ...
    return `
      <div class="bmc-cell" data-key="${q.key}">
        <div class="bmc-cell-h">${q.emoji} ${escapeHtml(q.title)}</div>
  ...
  sheet.innerHTML = `
    ...
    <div class="bmc-btns">
      <button class="plus-item" id="bmc-regen">🔄 Regénérer la stratégie</button>
      <button class="plus-item" id="bmc-edit">✏️ Modifier mes réponses</button>
    </div>
  `;
  $("#sheet-close").onclick = closeSheet;
