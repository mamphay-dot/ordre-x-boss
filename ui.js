...
function showView(v){
  ...
  // Log de consultation → activity_log (offline-safe)
  try { if(typeof ActivityLog !== "undefined") ActivityLog.log("view", v || "unknown", { profile: (cur() ? cur().id : null) }); } catch(_){}
}
...
const Cloud = (function(){
  ...
  async function pushLocal(){
    if(!available() || !session() || !orgId) return false;
    // OPTIMISATION E : diff — n'envoie que les profils modifiés depuis le dernier sync
    const lastSyncedAt = (state.cloud && state.cloud.lastSyncedAt) || 0;
    const toPush = {};
    let changedCount = 0;
    Object.keys(state.profiles).forEach(id => {
      const p = state.profiles[id];
      if((p.updatedAt || 0) > lastSyncedAt){
        toPush[id] = p;
        changedCount++;
      }
    });
    // Premier push ou aucune donnée modifiée → tout envoyer (ou rien)
    if(lastSyncedAt === 0){
      Object.assign(toPush, state.profiles);
      changedCount = Object.keys(toPush).length;
    }
    if(changedCount === 0){ setStatus("online"); return true; }

    // Enfile d'abord dans la queue résiliente (si le push échoue, l'entry reste)
    try { SyncQueue.enqueue({type:"pushProfiles", data:toPush, orgId, ts:Date.now(), count:changedCount}); } catch(_){}
    try {
      setStatus("syncing");
      await NET.profiles.pushMany(orgId, toPush);
      // Marque le sync réussi
      state.cloud = Object.assign({}, state.cloud, {lastSyncedAt: Date.now()});
      // Push réussi : on peut vider la queue des entries obsolètes de même type
      try { SyncQueue.drain("pushProfiles"); } catch(_){}
      setStatus("online");
      return true;
    } catch(e){
      setStatus("offline");
      // Entry reste dans SyncQueue, sera réessayée quand internet revient
      return false;
    }
  }
...
/* ============================================================
   ...
   ============================================================ */
/* ============================================================
   SyncQueue — queue résiliente des mutations non pushées
   Persiste dans IndexedDB, drain auto quand internet revient.
   ============================================================ */
const SyncQueue = (function(){
  const KEY = "boss:sync:queue";
  ...
  function saveQ(q){
    try {
      const trimmed = q.slice(-MAX_ENTRIES);
      localStorage.setItem(KEY, JSON.stringify(trimmed));
    } catch(_){}
    listeners.forEach(fn => { try { fn(trimmed || q); } catch(_){} });
  }
  function enqueue(entry){
    if(!entry) return;
    const q = loadQ();
    // Dédoublonnage : si même type que la dernière entry, on remplace (last-write-wins)
    if(q.length && q[q.length-1].type === entry.type){
      q[q.length-1] = { ...entry, id: q[q.length-1].id, retries: 0 };
    } else {
      q.push({ ...entry, id: "q_"+Date.now().toString(36)+Math.random().toString(36).slice(2,7), retries: 0 });
    }
    saveQ(q);
    // Enregistre un Background Sync (Chrome/Android) : le SW réveillera BOSS
    // pour drain la queue même si l'app est fermée
    try {
      if("serviceWorker" in navigator && "SyncManager" in window){
        navigator.serviceWorker.ready.then(reg => reg.sync.register("boss-drain-queue")).catch(()=>{});
      }
    } catch(_){}
  }
  function drain(type){
    // Vide la queue pour un type donné (appelé après un push réussi)
    const q = loadQ().filter(e => e.type !== type);
    saveQ(q);
  }
  function size(){ return loadQ().length; }
  function pending(){ return loadQ(); }
  function onChange(fn){ listeners.push(fn); return () => { listeners = listeners.filter(x=>x!==fn); }; }

  async function retryAll(){
    if(typeof Cloud === "undefined" || !Cloud.available || !Cloud.available() || !Cloud.session() || !Cloud.currentOrgId()) return;
    if(!navigator.onLine) return;
    const q = loadQ();
    if(!q.length) return;
    // Regroupe par type et rejoue le plus récent de chaque type
    const byType = {};
    q.forEach(e => { byType[e.type] = e; });
    for(const type of Object.keys(byType)){
      try {
        if(type === "pushProfiles"){
          await Cloud.pushLocal();
        }
      } catch(e){
        // Incrémente les retries, garde dans la queue
        const nq = loadQ().map(x => x.type === type ? { ...x, retries: (x.retries||0)+1 } : x);
        saveQ(nq);
      }
    }
  }
  ...
  // Auto-drain sur retour d'internet
  if(typeof window !== "undefined"){
    window.addEventListener("online", () => { setTimeout(retryAll, 500); });
    // Retour dans l'app (autre app fermée, écran déverrouillé)
    document.addEventListener("visibilitychange", () => {
      if(document.visibilityState === "visible") setTimeout(retryAll, 800);
    });
    // Retry périodique adaptatif : 30s si des entries en attente
    setInterval(() => { if(size() > 0 && isOnline()) retryAll(); }, 30000);
  }

  return { enqueue, drain, size, pending, onChange, retryAll, isOnline };
})();

/* ============================================================
   ActivityLog — trace consultations + actions clés (RGPD-safe)
   Écrit local d'abord, drainé vers Supabase via SyncQueue.
   ============================================================ */
const ActivityLog = (function(){
  const LOCAL_KEY = "boss:activity:queue";
  const MAX_LOCAL = 500;
  let sessionId = null;
  ...
  function getSessionId(){
    ...
    try {
      sessionId = sessionStorage.getItem("boss:session:id");
      if(!sessionId){
        sessionId = "s_"+Date.now().toString(36)+Math.random().toString(36).slice(2,8);
        sessionStorage.setItem("boss:session:id", sessionId);
      }
    } catch(_){ sessionId = "s_ephemeral"; }
    return sessionId;
  }
  function deviceKind(){
  ...
  function saveLocal(arr){
    try {
      const trimmed = arr.slice(-MAX_LOCAL);
      localStorage.setItem(LOCAL_KEY, JSON.stringify(trimmed));
    } catch(_){}
  }
  function log(type, name, meta){
    const entry = {
      event_type: type,
    ...
    // Ajoute au buffer local (même hors ligne)
    const arr = loadLocal(); arr.push(entry); saveLocal(arr);
    // Enfile un push pour drain quand internet + cloud
    try { SyncQueue.enqueue({type:"pushActivity", ts:Date.now()}); } catch(_){}
  }
  async function drain(){
    if(typeof Cloud === "undefined" || !Cloud.available() || !Cloud.session()) return false;
    const N = (typeof window !== "undefined" && window.BOSSNET) ? window.BOSSNET : null;
    if(!N) return false;
    const orgId = Cloud.currentOrgId ? Cloud.currentOrgId() : null;
    const arr = loadLocal();
    ...
    try {
      // Insert direct via l'API REST Supabase (bossnet expose fetch générique via db)
      // On utilise fetch directement pour éviter d'ajouter une méthode dans bossnet
      ...
      // Vide le buffer local (les rows sont côté serveur maintenant)
      saveLocal([]);
      return true;
    } catch(_){ return false; }
  }
  function pendingCount(){ return loadLocal().length; }
  return { log, drain, pendingCount, getSessionId };
})();

/* Drain périodique + sur online */
if(typeof window !== "undefined"){
  window.addEventListener("online", () => { setTimeout(()=>{ try { ActivityLog.drain(); } catch(_){} }, 1500); });
  setInterval(() => { if(navigator.onLine !== false) try { ActivityLog.drain(); } catch(_){} }, 60000);
}

/* ---------- Badge de sync dans la topbar ---------- */
function updateSyncBadge(){
  const badge = document.getElementById("cloud-badge");
  if(!badge) return;
  const isOnline = SyncQueue.isOnline();
  const pending = SyncQueue.size();
  const hasCloud = typeof Cloud !== "undefined" && Cloud.available && Cloud.available();
  const isSession = hasCloud && Cloud.session();

  badge.classList.remove("cb-off","cb-on","cb-sync","cb-warn","cb-err");
  if(!hasCloud){
    badge.classList.add("cb-off");
    badge.innerHTML = ic("cloud") + " Local";
    badge.title = "Espace en ligne non configuré (mode local pur)";
  } else if(!isSession){
    badge.classList.add("cb-off");
    badge.innerHTML = ic("user") + " Se connecter";
    badge.title = "Non connecté à l'espace en ligne";
  } else if(!isOnline){
    badge.classList.add("cb-warn");
    badge.innerHTML = ic("cloud") + (pending > 0 ? " " + pending : "");
    badge.title = pending > 0 ? `Hors-ligne : ${pending} modification(s) seront synchronisées à la reconnexion.` : "Hors-ligne";
  } else if(pending > 0){
    badge.classList.add("cb-sync");
    badge.innerHTML = ic("refresh") + " " + pending;
    badge.title = `${pending} modification(s) en cours de synchronisation…`;
  } else {
    badge.classList.add("cb-on");
    badge.innerHTML = ic("check");
    badge.title = "Synchronisé avec le cloud";
  }
}
...
if(typeof window !== "undefined"){
  ...
  try { SyncQueue.onChange(updateSyncBadge); } catch(_){}
...
window.addEventListener("DOMContentLoaded",async()=>{
  ...
  enforceLicense();
  IdleLock.start();
  // Badge de synchronisation dans la topbar : état temps réel
  try { updateSyncBadge(); setInterval(updateSyncBadge, 5000); } catch(_){}
  applyMenuCustomization();
  const p=cur();
  if(state.easyMode){
  ...
  if("serviceWorker" in navigator && location.protocol.indexOf("http")===0){
    try{
      ...
      // Le SW nous notifie quand un Background Sync se déclenche → drain
      navigator.serviceWorker.addEventListener("message", (ev)=>{
        if(ev.data && ev.data.type === "sync-drain"){
          try { SyncQueue.retryAll(); } catch(_){}
        }
      });
    }catch(e){}
  }
